...
| Google Search Console reports **Soft 404** on `/categories/` or `/tags/` | Hugo's default `tags`/`categories` taxonomies generate empty list pages (NL + EN) and list them in the sitemap. The site doesn't use taxonomies — paintings group on a plain `category` *string* param (`layouts/schilderijen/list.html`), not the taxonomy system. | Keep the empty `[taxonomies]` table in `hugo.toml` — it removes the default taxonomies so the empty pages are never generated. Verify with `hugo --minify` that `public/categories/` and `public/tags/` don't exist. Do **not** re-add taxonomies unless a real tag/category feature is built. Fixed 2026-06-15. |
| Google Search Console reports **Page with redirect** | Intentional and benign: `/nl/` is Hugo's noindex meta-refresh alias for the default-language home → `/`; plus `www`→apex and `http`→`https` 301s. Google indexes the targets. | No action. In GSC these can be left as-is or marked **Validate Fix** — they pass because the redirects are correct. |
